...
__global__ void searchb(char *data, char *pattern, int len_substring, bool *pos) {
  ...
  if (data[i] == pattern[0]) {
    // Loop through the next keyword character
    for (j = 1; j < len_substring; ++j) {
      if (data[i + j] != pattern[j])
        break;
    }
    pos[i] = j == len_substring ? true : false;
  }
} // end of Kernel

//
// Read in the given data file and hope it doesn't over the memory limits of the
// machine
//
...
unsigned long long filesize(const char *filename) {
  ...
  if (fseek(f, 0, SEEK_END) == 0) /* seek was successful: Linux */
  fileSize = ftell(f);
  fclose(f);
...
//Count all matching found function
...
int main(int argc, char **argv) {
  ...
  long dposSize = 0;
  int mb = 0; // pattern size of substring
  int nb = 0; // number of ints in the data set
  int j, k;
  ...
  while ((countc = fread(data_b, sizeof(char), (chunkSize + mb - 1), f_b)) >
         0) {
    ...
    cudaMemcpy(d_data_b, data_b, chunkSize,
               cudaMemcpyHostToDevice); //**maybe be Asynccopy
    ...
    for (j = 0; j < total_pattern; j++) {
      ...
      t_data1 = clock();
      cudaMemcpy(d_substr_b, pattern_arr[j],
      ...
      cudaEventRecord(start_event, 0);
          ...
          (d_data_b, d_substr_b, strlen(pattern_arr[j]), d_pos);
      ...
      t_data1 = clock();
      cudaMemcpy(pos, d_pos, nb, cudaMemcpyDeviceToHost); 
      t_data2 = clock();
      diff2 =(((long double)t_data2 - (double)t_data1) / CLOCKS_PER_SEC) * 1000;
      printf("timeCopyH2D-3rd %Lf ms \n", diff2);
      Rround++;
      total_diff2 += diff2;
